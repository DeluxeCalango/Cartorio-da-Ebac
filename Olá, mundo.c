...
int registrar() //Função responsável por cadastrar os usuários
{
	...
	char arquivo[40]; //Início da criação de strings
	char cpf[40];
	char nome[40];
	char sobrenome[40];
...
int consultar()
{
	...
	if(file == NULL)
	{
		printf("Não foi possível localizar o arquivo! (Inexistente).\n");
	}
	
	while (fgets(conteudo, 200, file) != NULL)
	{
	    printf("\nEssas são as informações do usuário em ordem: CPF, Nome, Sobrenome e Cargo\n\n");
		printf("%s", conteudo);
		printf("\n\n");
	}
...
int main()
{
	int opcao=0; //Espaço de armazenamento de escolha
	int laco=1;
